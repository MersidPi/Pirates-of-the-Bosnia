...
mqtt_conn = MQTTClient(client_id='PlayerA', server='broker.hivemq.com',user='',password='',port=1883)
mqtt_conn.set_callback(sub)
try:
    mqtt_conn.connect()
    mqtt_conn.subscribe(b"battleships/playerB")
    turnOffRunningLeds()
except OSError:
    turnOffRunningLeds()
    refreshDisplay()
