...
def aimPositionAttackSendUpdate():
    hit = False
    global x
    global y
    while True:        
        ...
        if input == "RIGHT" and x < len(matrixEnemy) - 1:
            ...
            sendMessage(b'battleships/playerB', bytes([7, y]))
    ...
    sendMessage(b'battleships/playerB', bytes([x, y]))
...
def defeatProcedure(matrix, startX, startY):
    ...
    display.print("Mission failed, we'll get 'em next time.")
    display.print("You can now see remaining enemy ships.")
...
def battle(matrix, inptMd):
    ...
    global inputMode
    inputMode = inptMd
    # count number of ships for this player B
    global myNumOfShips
    myNumOfShips = 0
    ...
    sendMessage(b'battleships/playerB', bytes(matrixMyValues))
    ...
    # determining which player goes first, player A will take care of everything
    myRandomNumber = [random.randint(1, 254)]
    sendMessage(b'battleships/playerB', bytes(myRandomNumber))
    
    # now goes battle
    while True:
        # this player B recieves attack from player A, or not, depending on random numbers
        hit = True
        while hit == True:
        ...
        hit = True
        while hit == True:
            hit = aimPositionAttackSendUpdate()
            sleep(0.5)
            # checking win/loss
            if myNumOfShips == 0:
                defeatProcedure(matrixEnemy, startMatrixEnemyX, startMatrixEnemyY)
